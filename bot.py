        ...
        try:
            ...
            if item:
                ...
                try:
                    ...
                    # Item is stuck in processing_dl_queue — move it back to avoid permanent loss
                    log.warning("redis_puller_queue_full_returning_item")
                    await REDIS.lrem("processing_dl_queue", 1, item)
                    await REDIS.rpush("global_dl_queue", item)
...
# ─────────────────────────────────────────────
#  FIX: SESSION CACHE MEMORY LEAK CLEANUP
#  Har 5 min mein expired sessions SESSION_MAP/EXPIRY/LAST_ACCESSED se remove karo
# ─────────────────────────────────────────────
async def session_cleanup_loop():
    while True:
        now = time.time()
        async with CACHE_LOCK:
            expired = [sid for sid, ts in SESSION_EXPIRY.items() if ts < now]
            for sid in expired:
                SESSION_MAP.pop(sid, None)
                SESSION_EXPIRY.pop(sid, None)
                SESSION_LAST_ACCESSED.pop(sid, None)
        if expired:
            log.info("session_cache_evicted", count=len(expired))
        await asyncio.sleep(300)
...
async def start_services():
    ...
    create_safe_task(session_cleanup_loop(), "session_cleanup")
